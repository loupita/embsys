...
int father(int fd1, int fd2, int pipe, char * port1, char * port2)
{
	...
	while(1)
	{
		...
		if (FD_ISSET(fd1, &fdset)) event(fd1, pipe, buff);
		if (FD_ISSET(fd2, &fdset)) event(fd2, pipe, buff);
		fflush(stdout);
	}
}
    ...
    while(1)
    {
        ...
        if (FD_ISSET(fd, &fdset))
        {
            ...
            if (bytes == 7)
            {
                ...
                time_str_s[2] = '\0';
                char message[BUFSIZ];
                sprintf(message, "Time: %sh %smin %ssec", time_str_h,time_str_m, time_str_s);
...
int main(int argc, char *argv [])
{
	...
	if (argc != 5)
	{
		fprintf(stderr, "Invalid usage: gps_reader -p first_port -s second_port\n");
		exit(EXIT_FAILURE);
	}
	char * options = "p:s:";
	int option;
